...
class RestartableMD4():
    ...
    def __init__(self, message, ml, A, B, C, D):
        self.A = A
        self.B = B
        self.C = C
        self.D = D
...
def md4_pad(message: bytes):
    ...
    message += bytes(i)
    message += length
    return b'\x80' + bytes(i) + length


def md4_restart(message:bytes, ml, A, B, C, D):
    return RestartableMD4(message, ml, A=A, B=B, C=C, D=D).hexdigest()


def md4(message: bytes) -> bytes:
    h = MD4(message).hexdigest()
    return h
